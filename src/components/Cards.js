...
export default function Cards({ tab, setTab }) {
  return (
    <div
      className="col-md-5 row overflow-auto p-1 pe-2 mx-2 accordion"
      style={{ height: "80vh", border: "1px solid #acabab" }}
    >
      {tab.map((id) => {
        return (
          <div
            id={"heading-" + id._id}
            key={"heading-" + id._id}
            className="card w-100 m-1 accordion-item px-0"
          >
            <h5 className="accordion-header">
              <button
                ...
                data-bs-target={"#collapse-" + id._id}
                aria-expanded="false"
            ...
            >
              <div className="accordion-body">{id.about}</div>
            </div>
          </div>
        );
      })}
