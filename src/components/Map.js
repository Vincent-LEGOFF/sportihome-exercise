...
export default function Map({ url, setURL, center, setCenter, tab, setTab }) {
  ...
  useEffect(() => {
    ...
    tab.map((id) => {
      ...
      popup.on("open", function () {
        ...
        document.querySelector("#heading-" + id._id + ">h5>button").className =
          "accordion-button collapsed p-2 ps-1 bg-info text-light";
      });

      popup.on("close", function () {
        if (document.querySelector("#heading-" + id._id + ">h5>button"))
          document.querySelector(
            "#heading-" + id._id + ">h5>button"
          ).className = "accordion-button collapsed p-2 ps-1";
      });

      var marker = new mapboxgl.Marker()
        .setLngLat(id.loc.coordinates)
        .setPopup(popup)
        .addTo(map);
      return marker;
    });
    ...
    document.querySelector("#refreshMap").onclick = () => {
      ...
      var link =
        "https://sportihome.com/api/spots/getAllMarkersInBounds/" +
        sw.lng +
        "," +
        sw.lat +
        "/" +
  ...
  return (
    <div className="w-100">
      <button
        id="refreshMap"
        className="btn btn-info text-light p-1 px-2 mb-1 text-align-right"
      >
        Refresh
      </button>
